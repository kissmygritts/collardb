...
names(devices) <- c('id', 'serial_number', 'purchase_date', 'frequency', 'vendor', 'model', 'sim')
names(devices)

## write table to the database
DBI::dbWriteTable(conn, 'devices', devices[, 1:6], append = TRUE)

## retreive the data
DBI::dbReadTable(conn, 'devices')
...
animals_up <- dplyr::select(animals, id, animal_id, name, species, sex, age = age_class)
animals_up
...
## in the device table
deploys <- dplyr::full_join(animals, devices, by = c('serial_num' = 'sensor_id')) %>%
  dplyr::select(animal_fk = id.x, devices_fk = id.y, inservice = start_time, outservice = end_time)
names(deploys)
deploys

lubridate::ymd_hms(deploys$inservice)
lubridate::ymd_hms()

strtrim(deploys$inservice)

DBI::dbListFields(conn, 'animals')
animals

## column name vector
DBI::dbAppendTable(conn, 'animals',
                   dplyr::select(animals, id, animal_id, name, species, sex, age = age_class, sex) %>%
                     dplyr::mutate(study = NA))

db_animals <- DBI::dbReadTable(conn, 'animals')

# how do I map a database table to a data frame? ----
animals
DBI::dbListFields(conn, 'animals')
names(animals)

nam_map <- DBI::dbListFields(conn, 'animals')
nam_map

df_names <- names(animals)
df_names
